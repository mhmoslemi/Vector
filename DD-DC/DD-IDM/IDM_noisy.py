...
# python3 -u IDM_cifar10.py --dataset CIFAR10 --model ConvNet --ipc 50 --dsa_strategy color_crop_cutout_flip_scale_rotate --init real --lr_img 0.2 --num_exp 5 --num_eval 5
#  --net_train_real --eval_interval 500 --outer_loop 1 --mismatch_lambda 0 --net_decay --embed_last 1000 --syn_ce --ce_weight 0.1 --train_net_num 1 --aug
...
def main():
    ...
    parser.add_argument('--save_path', type=str, default='result-IDM', help='path to save results')
    ...
    parser_bool(parser, 'net_train_real', True)
    ...
    parser.add_argument('--mismatch_lambda', type=float, default=0.0005, help='outer loop for network update')
    ...
    parser.add_argument('--embed_last', type=int, default=1000, help='outer loop for network update')
    ...
    parser.add_argument('--perturb_tensor_filepath', type=str, default='/home/mmoslem3/scratch/Unlearnable-Examples/experiments/CIFAR10_samplewise_min-min/perturbation.pt', help='Path to the saved perturbation.pt file')
    ...
    for exp in range(args.num_exp):
        ...
        labels_np = np.array([np.ones(args.ipc)*i for i in range(num_classes)])
        label_syn = torch.from_numpy(labels_np).to(dtype=torch.long, device=args.device).view(-1)
        ...
        for it in range(args.Iteration+1):
            ...
            if it == args.Iteration: 
                ...
                torch.save({'data': data_save, 'accs_all_exps': accs_all_exps, }, os.path.join(args.save_path, 'res_%s_%s_%s_%dipc-noise.pt'%(args.method, args.dataset, args.model, args.ipc)))
