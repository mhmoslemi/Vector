...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='/home/mmoslem3/scratch/UE-DD/data', help='dataset path')
    parser.add_argument('--save_path', type=str, default='result-dc', help='path to save results')
    ...
    eval_it_pool = [25,50,75,100, 120,150,200,250, 300]
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration+1):
            ...
            if it %10 ==0: #args.Iteration: # only record the final results
                data_save = ([copy.deepcopy(image_syn.detach().cpu()), copy.deepcopy(label_syn.detach().cpu())])
                torch.save({'data': data_save, 'accs_all_exps': accs_all_exps, }, os.path.join(args.save_path, 'res2_%s_%s_%s_%dipc.pt'%(args.method, args.dataset, args.model, args.ipc)))
