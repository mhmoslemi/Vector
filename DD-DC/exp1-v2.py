...
SAVE_PATH  = '/home/mmoslem3/scratch/UE-DD/extraEXP-DenseNet121'

# DGC_FILE = '/home/mmoslem3/scratch/UE-DD/result-fianle/res_CIFAR10_iter150_bug8_lamexcess0.5.pt'

DGC_FILE ='/home/mmoslem3/scratch/UE-DD/result-fianle/res_CIFAR10_iter150_bug8_lamexcess0.5.pt'
SW_FILE  = '/home/mmoslem3/scratch/Unlearnable-Examples-DD/noise-EMN/CIFAR10_SW.pt'
GUE_FILE = '/home/mmoslem3/scratch/Unlearnable-Examples-DD/DD-DC/gue_cifar10_SW.pt'
TUE_FILE = '/home/mmoslem3/scratch/Unlearnable-Examples-DD/DD-DC/TUE_simclr_cifar10.pt'
...
def _load_sw_noise(noise_file, imgs, labs, mean, std, device):
    """Apply sample-wise noise from a .pt file to normalized images."""
    ...
    return (torch.clamp(imgs * std_t + mean_t + noise, 0.0, 1.0) - mean_t) / std_t
...
def load_poisoned(noise_type, mean, std, dst_train, device, num_classes):
    if noise_type == 'DGC':
        ...
        noise_file = {'SW': SW_FILE, 'GUE': GUE_FILE, 'TUE': TUE_FILE}[noise_type]
        imgs = _load_sw_noise(noise_file, imgs, labs, mean, std, device)
...
def main():
    ...
    results = {}
    for noise_type in ('clean', 'DGC', 'SW', 'GUE', 'TUE'):
    ...
    # ── Plot: training loss ────────────────────────────────────────────────
    colors = {'clean': 'green', 'DGC': 'royalblue', 'SW': 'tomato', 'GUE': 'darkorange', 'TUE': 'mediumpurple'}
    epochs = np.arange(1, EMN_EPOCHS + 1)
    cosine_epochs = np.arange(0, EMN_EPOCHS, PROBE_FREQ) + 1

    SMOOTH_W = 7  # moving-average window for cosine plot (odd number works best)
    ...
    for nt, (_, cc) in results.items():
        ax.plot(cosine_epochs, smooth(cc, SMOOTH_W), color=colors[nt], label=nt, linewidth=1.8)
