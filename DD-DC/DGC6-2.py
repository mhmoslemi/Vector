...
def main():
    ...
    parser.add_argument('--lr_img', type=float, default=1.5)
    ...
    parser.add_argument('--inner_loop', type=int, default=4)
    parser.add_argument('--outer_loop', type=int, default=25)


    parser.add_argument('--EMN_EPOCHS', type=float, default=15)
    ...
    for ee in [args.budget]:
        ...
        subset_save_path = os.path.join(args.save_path, f'subset_{args.dataset}_{subset_frac:.2f}.pt')
        torch.save({'indices': keep_idx, 'dataset': dst_train}, subset_save_path)
        print(f'Saved subset to {subset_save_path}')
        ...
        resume_path = os.path.join(args.save_path, 'cifar10-avg.pt')
        if os.path.exists(resume_path):
            print(f'Resuming from checkpoint: {resume_path}')
            ckpt = torch.load(resume_path, map_location=args.device, weights_only=False)
            images_poisoned = ckpt['images_poisoned'].to(args.device).detach().requires_grad_(True)
        else:
            print('No checkpoint found, initializing from clean images.')
        ...
        for it in range(args.Iteration + 1):
            ...
            for ol in range(args.outer_loop):
                ...
                # OUTER LOOP: Iterate over TRUE classes to isolate gradient matching per class
                for c in range(num_classes):
                    ...
                    # --- 1. PREPARE TARGET GRADIENTS ---
                    # Must use the CLEAN image (or slightly noisy clean image) for the target
                    output_real = net(img_real) 

                    class_grads = []


                    # INNER LOOP: Iterate over every possible target class label
                    for target_class in range(num_classes):
                        ...
                        # Create a target tensor where every sample is assigned 'target_class'
                        target_c = torch.full_like(lab_real, target_class)
                        
                        # Calculate loss for this specific incorrect class assignment
                        loss_c = criterion(output_real, target_c)
                        
                        # Retain graph to reuse output_real in the next loop iteration
                        g_c = torch.autograd.grad(loss_c, net_parameters, retain_graph=True)
                        class_grads.append(list((_.detach().clone() for _ in g_c)))

                    # Construct BALANCED Target Gradient (Average across the C-1 incorrect classes)
                    gw_target = []
                    for i in range(len(net_parameters)):
                        # class_grads now has exactly (num_classes - 1) items
                        layer_grads = [grads[i] for grads in class_grads]
                        gw_target.append(torch.stack(layer_grads).mean(dim=0))


                    # --- 2. COMPUTE POISON GRADIENTS ---
                    if args.dsa:
                        seed = int(time.time() * 1000) % 100000
                        img_poison = DiffAugment(img_poison, args.dsa_strategy, seed=seed, param=args.dsa_param)

                    # Use the POISONED image for the actual gradient we are optimizing
                    ...
                    # --- 3. DIRECTION MATCHING & REGULARIZATION ---
                ...
                # 4. Step the optimizer ONCE after accumulating all class losses
            ...
            if it % 5 ==0 :
                ...
                }, os.path.join(args.save_path, 'cifar10-avg.pt' ))
