...
def main():
    ...
    parser.add_argument('--EMN_EPOCHS', type=float, default=10)
    ...
    for ee in [args.budget]:
        ...
        grad_dataset = torch.utils.data.TensorDataset(
            images_all,
            labels_all,
            torch.arange(len(images_all), device=args.device),
        )
        grad_loader = torch.utils.data.DataLoader(
            grad_dataset, batch_size=args.batch_real, shuffle=True, drop_last=True, num_workers=0
        )
        ...
        for it in range(args.Iteration + 1):
            ...
            for ol in range(args.outer_loop):
                ...
                try:
                    img_real, lab_real, gidx = next(grad_iter)
                except StopIteration:
                    grad_iter = iter(grad_loader)
                    img_real, lab_real, gidx = next(grad_iter)
                img_poison = images_poisoned[gidx]

                # 1. Chaotic target gradient — clean images + random wrong labels
                lab_random = (lab_real + torch.randint(1, num_classes, (len(lab_real),), device=args.device)) % num_classes
                ...
                for _ in range(args.inner_loop):
                    ...
                    g = torch.Generator(device=args.device).manual_seed(int(time.time() * 1000) % (2**31) + ol)
            ...
            loss_match_avg /= args.outer_loop
            loss_reg_avg /= args.outer_loop
            if it % 1== 0:
            ...
            #     }, os.path.join(args.save_path, 'cifar10-partial_'+str(int(100*subset_frac))+'.pt' ))
