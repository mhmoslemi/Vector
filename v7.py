...
def target_gradient(model, x_u, y, criterion, eps, r, inner_steps, use_proxy):
    """
    If use_proxy is True: G_target = grad(l(x' + delta*; theta)) evaluated EXACTLY
        at the restored point. delta* is obtained from PGD ascent on the loss
        (eq. 39). No Taylor expansion, no r->0 finite difference, no HVP.
    If False: G_target = grad(l(x'; theta))   (vanilla DC)
    """
    if not use_proxy:
...
def main():
    ...
    p.add_argument("--pert", type=str, default="perturbation1.pt")
    p.add_argument("--data-root", type=str, default="data")
    
    # p.add_argument("--ipc", type=int, default=3000)
    p.add_argument("--ipc", type=int, default=2000)
    ...
    # p.add_argument("--epochs-eval", type=int, default=80)
    # p.add_argument("--epochs-eval-mid", type=int, default=80)
    ...
    p.add_argument("--plot", type=str, default="cpdd_v4_combined.png")
    ...
    p.add_argument("--rerun", type=str, nargs="*", default=['clean'],
                   choices=["clean"],
