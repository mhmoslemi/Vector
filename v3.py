...
Structurally identical to DC (Zhao et al. 2021):
  - one fresh net per outer iteration `it`
  - inside each `it`: `outer_loop` rounds of {match-and-update-syn, train-net-on-syn}
  - net is trained on the current synthetic set for `inner_loop` SGD epochs
    between matching rounds (this is the part that was missing in v2 and is
    why the matching loss looked like white noise: the net never moved)

The ONLY deviation from DC is the target gradient. Instead of
    gw_real = grad(loss(x_real; theta))
we use the eq. 42 proxy
    gw_real = grad(loss(x'; theta))
             + (grad(loss(x' + r * delta*; theta)) - grad(loss(x'; theta))) / r
where delta* is obtained by L_inf PGD ascent on loss (eq. 39), recomputed
fresh against the current `theta` every match step.

Plotting: 3 seeds for each evaluation point, mean +/- 1 std band.
"""
...
# ----------------------------- DC loop schedule -----------------------------
def get_loops(ipc):
    """DC paper schedule: (outer_loop, inner_loop) as a function of ipc."""
    if ipc == 1:    return 1, 1
    ...
    if ipc == 100:  return 50, 10  # extrapolated; tune if needed
...
def model_grad(model, x, y, criterion):
    """Detached parameter gradient. Used for both ends of the FD HVP."""
...
def model_grad_with_graph(model, x, y, criterion):
    """Synthetic-side gradient with create_graph=True so the matching loss
    can be backpropped into syn pixels."""
    out = model(normalize(x))
    loss = criterion(out, y)
    grads = torch.autograd.grad(loss, list(model.parameters()),
...
def restorative_perturbation(model, x, y, criterion, eps, n_steps=10, alpha=None):
    """L_inf PGD ascent on the loss. Returns delta* with ||delta*||_inf <= eps."""
    if alpha is None:
...
def target_gradient(model, x_u, y, criterion, eps, r, inner_steps=10):
    """
    G_target = grad(l(x'; theta)) + ( grad(l(x'+r*delta*; theta)) - grad(l(x'; theta)) ) / r
    """
...
def train_net_on_syn(net, image_syn, label_syn, args):
    """
    DC inner loop: train `net` for args.inner_loop epochs of plain SGD on the
    current synthetic dataset. This is what makes the matching meaningful: by
    the next outer round, theta has actually moved along the synthetic gradient
    flow, so matching gradients at the new theta is informative.
    """
...
def distill(x_u, y_u, num_classes, ipc, args, device, x_te=None, y_te=None):
    # init synthetic from real (poisoned) samples, ipc per class
    s_imgs, s_labels = [], []
    ...
    # cache class indices for fast sampling
    ...
    for it in range(args.Iteration + 1):
        # one fresh randomly initialized net per outer iteration (DC convention)
        ...
        for ol in range(args.outer_loop):
            ...
            for c in range(num_classes):
                ...
                # eq. 42 target proxy at the CURRENT theta
            ...
            # last outer round in this iter: don't bother retraining the net
            if ol == args.outer_loop - 1:
                break

            # ----- update network on current synthetic data (DC inner loop) -----
