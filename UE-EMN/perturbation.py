...
parser.add_argument('--subset_pt', default=None, type=str, help='Path to .pt file saved by DGC6.py containing subset indices')
parser.add_argument('--skew_ratio', default=0.9, type=float, help='Bias skew ratio for CIFAR10_S')
parser.add_argument('--severity', default=0, type=int, help='Corruption severity for CIFAR10_S minority group (0-5)')
args = parser.parse_args()

# Load subset indices if provided
_subset_indices = None
if args.subset_pt is not None:
    _ckpt = torch.load(args.subset_pt, map_location='cpu', weights_only=False)
    _subset_indices = _ckpt['indices']
    print(f'Loaded subset indices from {args.subset_pt}: {len(_subset_indices)} samples')
    # For samplewise noise the first dim is one-per-sample; adapt to subset size
    if args.perturb_type == 'samplewise':
        args.noise_shape[0] = len(_subset_indices)
        print(f'noise_shape adapted to subset size: {args.noise_shape}')
