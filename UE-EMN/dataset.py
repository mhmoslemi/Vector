...
transform_options['PoisonMNIST'] = transform_options['MNIST']
transform_options['PoisonFashionMNIST'] = transform_options['FashionMNIST']
transform_options['PoisonCIFAR10'] = transform_options['CIFAR10']
transform_options['CIFAR10_S'] = transform_options['CIFAR10']
...
class DatasetGenerator():
    def __init__(self, train_batch_size=128, eval_batch_size=256, num_of_workers=1,
                 ...
                 use_cutout=None, use_cutmix=False, use_mixup=False, subset_indices=None,
                 skew_ratio=0.9, severity=0):
        ...
        if train_data_type == 'CIFAR10':
            ...
            test_dataset = datasets.CelebA(root=train_data_path, split="all",
                                           target_type="identity", transform=test_transform)
        elif train_data_type == 'MNIST':
            num_of_classes = 10
            train_dataset = datasets.MNIST(root=train_data_path, train=True,
                                           download=True, transform=train_transform)
        elif train_data_type == 'PoisonMNIST':
            num_of_classes = 10
            train_dataset = PoisonMNIST(root=train_data_path, transform=train_transform,
                                        poison_rate=poison_rate, perturb_type=perturb_type,
                                        patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                        perturb_tensor_filepath=perturb_tensor_filepath,
                                        add_uniform_noise=add_uniform_noise,
                                        poison_classwise=poison_classwise,
                                        poison_classwise_idx=poison_classwise_idx)
        elif train_data_type == 'FashionMNIST':
            num_of_classes = 10
            train_dataset = datasets.FashionMNIST(root=train_data_path, train=True,
                                                  download=True, transform=train_transform)
        elif train_data_type == 'PoisonFashionMNIST':
            num_of_classes = 10
            train_dataset = PoisonFashionMNIST(root=train_data_path, transform=train_transform,
                                               poison_rate=poison_rate, perturb_type=perturb_type,
                                               patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                               perturb_tensor_filepath=perturb_tensor_filepath,
                                               add_uniform_noise=add_uniform_noise,
                                               poison_classwise=poison_classwise,
                                               poison_classwise_idx=poison_classwise_idx)
        elif train_data_type == 'CIFAR10_S':
            num_of_classes = 10
            train_dataset = CIFAR10SWrapper(root=train_data_path, split='train',
                                            transform=train_transform,
                                            skew_ratio=skew_ratio, severity=severity)
        else:
        ...
        #     pass
        # elif test_data_type in ('CelebAMini', 'CelebA'):
        #     pass

        # elif test_data_type == 'FaceScrub' or test_data_type == 'WebFace':
        #     pass
        elif test_data_type == 'MNIST':
            test_dataset = datasets.MNIST(root=test_data_path, train=False,
                                          download=True, transform=test_transform)
        elif test_data_type == 'PoisonMNIST':
            test_dataset = PoisonMNIST(root=test_data_path, train=False, transform=test_transform,
                                       poison_rate=poison_rate, perturb_type=perturb_type,
                                       patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                       perturb_tensor_filepath=perturb_tensor_filepath,
                                       add_uniform_noise=add_uniform_noise,
                                       poison_classwise=poison_classwise,
                                       poison_classwise_idx=poison_classwise_idx)
        elif test_data_type == 'FashionMNIST':
            test_dataset = datasets.FashionMNIST(root=test_data_path, train=False,
                                                 download=True, transform=test_transform)
        elif test_data_type == 'PoisonFashionMNIST':
            test_dataset = PoisonFashionMNIST(root=test_data_path, train=False, transform=test_transform,
                                              poison_rate=poison_rate, perturb_type=perturb_type,
                                              patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                              perturb_tensor_filepath=perturb_tensor_filepath,
                                              add_uniform_noise=add_uniform_noise,
                                              poison_classwise=poison_classwise,
                                              poison_classwise_idx=poison_classwise_idx)
        elif test_data_type == 'CIFAR10_S':
            test_dataset = CIFAR10SWrapper(root=test_data_path, split='test',
                                           transform=test_transform,
                                           skew_ratio=skew_ratio, severity=severity)
        else:
        ...
        if subset_indices is not None:
            ...
            if train_data_type in ('MNIST', 'PoisonMNIST', 'FashionMNIST', 'PoisonFashionMNIST'):
                train_dataset.data = train_dataset.data[idx]
                train_dataset.targets = train_dataset.targets[idx]
            elif train_data_type in ('SVHN', 'PoisonSVHN'):
                train_dataset.data = train_dataset.data[idx]
                train_dataset.labels = [train_dataset.labels[i] for i in idx]
            elif train_data_type in ('ImageNetMini', 'TinyImageNet', 'PoisonImageNetMini'):
                train_dataset.samples = [train_dataset.samples[i] for i in idx]
                train_dataset.targets = [train_dataset.targets[i] for i in idx]
            elif train_data_type == 'CIFAR10_S':
                train_dataset._inner.dataset['image'] = train_dataset._inner.dataset['image'][idx]
                train_dataset._inner.dataset['label'] = train_dataset._inner.dataset['label'][idx]
                train_dataset._inner.dataset['color'] = train_dataset._inner.dataset['color'][idx]
            else:
                train_dataset.data = np.array(train_dataset.data)[idx]
                train_dataset.targets = [train_dataset.targets[i] for i in idx]
            print(f'DatasetGenerator: training set restricted to {len(idx)} samples via subset_indices')
    ...
    def _split_validation_set(self, train_portion, train_shuffle=True, train_drop_last=True):
        ...
        elif self.train_data_type in ('MNIST', 'PoisonMNIST', 'FashionMNIST', 'PoisonFashionMNIST'):
            raw_data = self.datasets['train_dataset'].data
            raw_targets = self.datasets['train_dataset'].targets
            if torch.is_tensor(raw_data):
                raw_data = raw_data.numpy()
            if torch.is_tensor(raw_targets):
                raw_targets = raw_targets.numpy()
            datasplit = train_test_split(raw_data, raw_targets, test_size=1-train_portion,
                                         train_size=train_portion, shuffle=True, stratify=raw_targets)
            train_D, valid_D, train_L, valid_L = datasplit
            print('Train Labels: ', np.array(train_L))
            print('Valid Labels: ', np.array(valid_L))
            train_subset.data = torch.from_numpy(np.array(train_D))
            valid_subset.data = torch.from_numpy(np.array(valid_D))
            train_subset.targets = torch.tensor(train_L)
            valid_subset.targets = torch.tensor(valid_L)
        else:
...
class CIFAR10SWrapper(Dataset):
    """Wraps CIFAR_10S (biased CIFAR-10) to return (image, label) pairs.

    CIFAR_10S returns (image, label, color, index_tuple); this wrapper drops
    the extra fields so it is compatible with perturbation.py's data loops.
    """
    _DD_DC_PATH = os.path.join(os.path.dirname(__file__), '..', 'DD-DC')

    def __init__(self, root, split='train', transform=None, skew_ratio=0.9, severity=0, seed=0):
        import sys
        dd_dc = os.path.normpath(self._DD_DC_PATH)
        if dd_dc not in sys.path:
            sys.path.insert(0, dd_dc)
        from cifar10 import CIFAR_10S
        self._inner = CIFAR_10S(root=root, split=split, transform=None,
                                seed=seed, skewed_ratio=skew_ratio, severity=severity)
    ...
    def __getitem__(self, index):
        image, label, _color, _idx = self._inner[index]
...
class PoisonMNIST(datasets.MNIST):
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=True, poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='classwise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None):
        super(PoisonMNIST, self).__init__(root=root, train=train, download=download,
        ...
        self.patch_location = patch_location
        self.img_denoise = img_denoise
        # MNIST data is stored as a uint8 tensor (N, H, W); convert to float32 numpy for arithmetic
        ...
        if poison_classwise:
            ...
            if poison_classwise_idx is None:
                ...
                self.poison_class = poison_classwise_idx
            self.poison_samples_idx = []
            for i, label in enumerate(self.targets.tolist()):
                if label in self.poison_class:
                    self.poison_samples_idx.append(i)
        else:
            targets = list(range(0, len(self)))
        ...
        for idx in self.poison_samples_idx:
            ...
            if perturb_type == 'samplewise':
                noise = perturb_tensor[idx]
                noise = patch_noise_extend_to_img(noise, [28, 28, 1], patch_location=self.patch_location)
            elif perturb_type == 'classwise':
                noise = perturb_tensor[label]
                noise = patch_noise_extend_to_img(noise, [28, 28, 1], patch_location=self.patch_location)
            if add_uniform_noise:
                noise += np.random.uniform(0, 8, (28, 28, 1))
            # noise is (H, W, 1); MNIST data is (H, W) — squeeze channel dim
            self.data[idx] = self.data[idx] + noise[:, :, 0]
            self.data[idx] = np.clip(self.data[idx], a_min=0, a_max=255)

        # Convert back to uint8 tensor so parent __getitem__ can call .numpy() on it
        self.data = torch.from_numpy(self.data.astype(np.uint8))
        print('add_uniform_noise: ', add_uniform_noise)
        print(self.perturb_tensor.shape)
        print('Poison samples: %d/%d' % (len(self.poison_samples), len(self)))


class PoisonFashionMNIST(datasets.FashionMNIST):
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=True, poison_rate=1.0, perturb_tensor_filepath=None,
                 seed=0, perturb_type='classwise', patch_location='center', img_denoise=False,
                 add_uniform_noise=False, poison_classwise=False, poison_classwise_idx=None):
        super(PoisonFashionMNIST, self).__init__(root=root, train=train, download=download,
                                                 transform=transform, target_transform=target_transform)
        self.perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
        if len(self.perturb_tensor.shape) == 4:
            self.perturb_tensor = self.perturb_tensor.mul(255).clamp_(0, 255).permute(0, 2, 3, 1).to('cpu').numpy()
        ...
        self.patch_location = patch_location
        self.img_denoise = img_denoise
        # FashionMNIST data is stored as a uint8 tensor (N, H, W); convert to float32 numpy for arithmetic
        self.data = self.data.numpy().astype(np.float32)
        ...
        for idx in self.poison_samples_idx:
            ...
            if add_uniform_noise:
                noise += np.random.uniform(0, 8, (28, 28, 1))
            # noise is (H, W, 1); FashionMNIST data is (H, W) — squeeze channel dim
            self.data[idx] = self.data[idx] + noise[:, :, 0]
            self.data[idx] = np.clip(self.data[idx], a_min=0, a_max=255)
